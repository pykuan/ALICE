...
TF1* Boltzmann = new TF1("Boltzmann", "500 * x * TMath::Exp( -(TMath::Sqrt(0.13957^2+x^2))/0.44 )", 0, 5);
TH1F *h_dndpt_nu = new TH1F("h_dndpt_nu", "dndpt_nu", nBins, 0, 5);
TH1F *h_dndpt_u = new TH1F("h_dndpt_u", "dndpt_u", nBins, 0, 5);
TH1F *h_weight = new TH1F("h_weight", "weight", nBins, 0, 5);
...
void Acceptance()
{
  ...
  for(int k=0; k<nPar_acc; k++){
    ...
    if(pt>=0.4 && pt<1.2){
      if(bKeep){h_dndpt_nu->Fill(pt);}
    }
    else {h_dndpt_nu->Fill(pt);}

  }

  //weights histogram
  for(int i=1; i<nBins+1; i++){
    h_weight->SetBinContent(i, (h_dndpt_nu->GetBinContent(i)>0) ? (h_dndpt_u->GetBinContent(i)/h_dndpt_nu->GetBinContent(i)) : 0.0);
  }

  pdf->Write(pdf->GetName(), TObject::kSingleKey + TObject::kOverwrite);
  Boltzmann->Write(Boltzmann->GetName(), TObject::kSingleKey + TObject::kOverwrite);
  h_dndpt_nu->Write(h_dndpt_nu->GetName(), TObject::kSingleKey + TObject::kOverwrite);
  h_dndpt_u->Write(h_dndpt_u->GetName(), TObject::kSingleKey + TObject::kOverwrite);
...
void TrueHarmonic(){
  ...
  ydata1[2] = pow(vn[1], 2)*pow(vn[4], 2) / pow(10., -4); //<4>
  ydata1[3] = vn[0]*pow(vn[3], 2)*vn[4]*vn[5] / pow(10., -5); //<5>
  ydata1[4] = pow(vn[1], 4)*vn[2]*vn[4] / pow(10., -6); //<6>
  ydata1[5] = vn[0]*pow(vn[1], 5)*vn[2] / pow(10., -7); //<7>
  ydata1[6] = pow(vn[1],4)*pow(vn[3], 2)*pow(vn[4], 2) / pow(10., -8); //<8>
}//not used

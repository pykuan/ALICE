...
int question6(){
    ...
    double v2 = 0.;
    TArrayD* phi_array = new TArrayD(nParticle);
    ...
    for(int i=0; i<nEvent; i++){
        ...
        for(int j=0; j<nParticle; j++){
            phi = f->GetRandom();
            phi_array->SetAt(phi, j);
        }
        v2_over_particle = 0;
        for(int i1=0; i1<nParticle; i1++){
            for(int i2=0; i2<nParticle; i2++){
                v2_over_particle += (i1==i2) ? 0 : TMath::Cos(2*(phi_array->At(i1)-phi_array->At(i2)))/(nParticle*nParticle-nParticle);                
            }
        }
